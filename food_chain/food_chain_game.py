...
from .food_chain_board import FoodChainBoard
...
class FoodChainGame:
    def __init__(self):
        ...
        self.bank_money = None
        self.bank_broke = False
        self.phase = None
    ...
    def start(self):
        self.bank_money = 50 * len(self.players)
    ...
    def get_possible_moves(self, player=None):
        ...
        if player:
            if player.get_player_id() != self.get_current_player_id():
                return []
        cur_player = self.get_current_player()
        if not cur_player.first_restaurant_placed:
            for possible_entrance in self.board.get_possible_restaurant_entrances():
                legal_moves.append(FoodChainMove(location=possible_entrance, player=player, type="place"))
    ...
    def is_over(self):
        return self.bank_broke and self.bank_money == 0
